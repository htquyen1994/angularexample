...
export class MeasureTool {
  ...
  _startElementNode: HTMLLIElement;
  _endElementNode: HTMLLIElement;
  _tooltip: Tooltip;
  _firstClick: any;
  _projection: any;
  _started: any;
  ...
  _projectionUtility: ProjectionUtility;
  _svgOverlay: any;
  _linesBase: any;
  _linesAux: any;
  _segmentText: any;
  ...
  _dragged: any;
  _dragging: any;
  _zoomControl: boolean;
  _lastMeasure: any;
  _disableAddNode: boolean = false;
  ...
  get segments() { return _.cloneDeep(this._segments) || []; };
  get points() { return _.cloneDeep(this._geometry.nodes.map(x => ({ lat: x[1], lng: x[0] }))) || []; };
  ...
  _init() {
    ...
    if (this._options.contextMenu) {
      ...
      this._startElementNode = this._contextMenu.addItem("Measure distance", true, this.start);
      this._endElementNode = this._contextMenu.addItem("Clear measurement", false, this.end);
  ...
  disableAddNode(value: boolean){
    this._disableAddNode = value;
  }
  ...
  _checkClick(mouseEvent) {
    ...
    if (!this._dragged && this._nodeCircles.selectAll('circle[r="6"]').size() == 0 &&
      !this._hoverCircle.select("circle").attr('cx') && !this._disableAddNode) {
  ...
  _updateNodeText() {
    let text = this._nodeText.selectAll("text")
      ...
      .text((d, i) => {
        ...
        return i == 0 ? this._options.label : '' +this._helper.formatLength(len);
      });
  ...
  _updateArea(i, pointToCompare) {
    ...
    if (area > 0) {
      this._nodeText.select(':last-child')
        .text(`Distance: ${this.lengthText} (Area: ${this.areaText}).`);
    }
  }
